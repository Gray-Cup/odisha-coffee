 ...
 */
export async function createReviewUploadPost(contentType: string, env: Env) {
  if (!ALLOWED_CONTENT_TYPES.includes(contentType)) {
    throw new Error("Only JPEG, PNG, or WebP images are allowed.");
  }
  const bucket = env.B2_BUCKET_NAME;
  const publicBase = env.B2_PUBLIC_BASE_URL;
  ...
  const { url, fields } = await createPresignedPost(getB2Client(env), {
    Bucket: bucket,
    Key: key,
    Conditions: [
