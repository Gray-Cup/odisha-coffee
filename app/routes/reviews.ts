...
import { ensureReviewsTable, reviews, getTursoDb } from "@/db/turso";
...
export async function action({ request, context }: Route.ActionArgs) {
  ...
  if (!reviewerName) return Response.json({ error: "Name is required." }, { status: 400 });
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
  ...
  if (images.length > MAX_REVIEW_IMAGES) {
    return Response.json({ error: `Up to ${MAX_REVIEW_IMAGES} photos allowed.` }, { status: 400 });
  }
  const publicBase = env.B2_PUBLIC_BASE_URL?.replace(/\/$/, "");
