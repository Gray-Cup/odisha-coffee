...
function BuilderContent() {
  ...
  useEffect(() => {
    ...
    navigate(`/order-builder${qs}`, { replace: true, preventScrollReset: true });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [items]);
  ...
  function proceedToCheckout() {
    ...
    navigate(`/checkout?products=${encodeURIComponent(productsParam)}&total=${total}`);
  }
                    ...
                    return (
                      <div key={`${item.productId}:${item.weight}:${item.farmId ?? ""}`} className="flex items-center gap-3">
                        <div className="w-12 h-12 border-2 border-odisha-black shrink-0 overflow-hidden relative bg-odisha-offwhite">
                          {image && <img src={image} alt={resolved.product.name} className="absolute inset-0 h-full w-full object-cover" />}
                        </div>
