...
// Fully client-rendered - no loader, since the previous Next.js version was
// already client-only. 404s render inline rather than via a route
// ErrorBoundary since there's no loader to throw from.

function NotFoundBlock() {
  return (
    <div className="max-w-7xl mx-auto px-4 lg:px-6 py-24 text-center">
      <h1 className="font-serif text-2xl font-bold text-odisha-black mb-2">Product not found</h1>
...
export default function FarmProductDetailPage() {
  ...
  const product = productSlug ? getEstateProductById(productSlug) : undefined;
  ...
  if (!farm || !product || !slug || !productSlug) return <NotFoundBlock />;
  if (product.exclusiveFarmId && product.exclusiveFarmId !== slug) return <NotFoundBlock />;
  if (!selectedWeight) return <NotFoundBlock />;
      ...
      <section className="bg-odisha-red pattachitra-pattern-red border-b-2 border-odisha-black">
        <div className="max-w-7xl mx-auto px-4 lg:px-6 py-12">
          <div className="flex items-center gap-2 mb-6 text-xs flex-wrap">
            ...
            <span className="text-white/30">/</span>
            <Link to={`/farms/${slug}/green-coffee`} className="text-white/60 hover:text-white uppercase tracking-widest transition-colors">Green Coffee</Link>
            <span className="text-white/30">/</span>
            ...
            <div className="space-y-4">
              ...
              <div className="border-2 border-odisha-black bg-white sticky top-4">
                ...
                <div className="p-5 space-y-5">
                  ...
                  {/* Contact form link */}
                  <Link
                    to={`/contact?farm=${slug}&product=${productSlug}&qty=${selectedWeight.label}`}
